...
filter ::
  ...
  -> List a
  -> List a
filter predicate = foldRight foldFn Nil
  where foldFn v acc
          | predicate v = v :. acc
          | otherwise   = acc
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap fn = foldLeft (\acc x -> acc ++ fn x) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight foldingFunction (Full Nil)
  where
    foldingFunction (Full x) (Full xs) = Full (x:.xs)
    foldingFunction _ _ = Empty
...
find fn = find'
  where
    find' (x :. xs)
      ...
      | otherwise = find' xs
    find' Nil = Empty
-- find fn = foldRight foldingFunction Empty
--   where
--     foldingFunction x Empty
--       | fn x = Full x
--       | otherwise = Empty
--     foldingFunction _ y = y
...
produce ::
  ...
  -> a
  -> List a
produce fn a = (a :. Nil) ++ produce fn next
  where
    next = fn a
